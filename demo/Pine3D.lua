...
function loadModel(path)
	...
	if not modelFile then error("Could not find model file '" .. path .. "'") end
	local content = modelFile.readAll()
	modelFile.close()
	if not content then error("Model '" .. path .. "' was empty") end
	...
	if not model then error("Could not unserialise model from '" .. path .. "'") end
...
local function newFrame(x1, y1, x2, y2)
	...
	function frame:newObject(model, x, y, z, rotX, rotY, rotZ)
		...
		elseif type(model) == "string" then
			...
			error("Model must be either a model table or a filename string")
		end
